...
Public Class FrmPetugas
    ...
    Sub tampildata()
        ...
        DataGridView1.Columns(4).HeaderText = "Hak"
    ...
    Sub kodeotomatis()
        ...
        cn.Open()
        cmd.Connection = cn
        cmd.CommandText = "SELECT COUNT(*) AS id FROM petugas"
        Dim rd As SqlDataReader = cmd.ExecuteReader
        While rd.Read
    ...
    Private Sub btnsimpan_Click(sender As Object, e As EventArgs) Handles btnsimpan.Click
        ...
        ElseIf txtuser.Text <> "" And txtpass.Text <> "" Then
            cn.Open()
            cmd.Connection = cn
            cmd.CommandText = "INSERT INTO petugas VALUES ('" & txtid.Text & "','" & txtuser.Text & "','" & txtpass.Text & "','" & txtnama.Text & "','" & txthak.Text & "') "
            cmd.ExecuteNonQuery()
            cn.Close()
            bersih()
    ...
    Private Sub btnubah_Click(sender As Object, e As EventArgs) Handles btnubah.Click
        ...
        ElseIf txtuser.Text <> "" And txtpass.Text <> "" Then
            cn.Open()
            cmd.Connection = cn
            cmd.CommandText = "UPDATE petugas SET username ='" & txtuser.Text & "' , password ='" & txtpass.Text & "' , nama_petugas ='" & txtnama.Text & "' , hak ='" & txthak.Text & "' WHERE id_petugas ='" & txtid.Text & "'"
            cmd.ExecuteNonQuery()
            cn.Close()
            bersih()
    ...
    Private Sub txtcari_TextChanged(sender As Object, e As EventArgs) Handles txtcari.TextChanged
        cn.Open()
        cmd.Connection = cn
        cmd.CommandText = "SELECT * FROM petugas WHERE id_petugas LIKE '%" & txtcari.Text & "%' OR username LIKE '%" & txtcari.Text & "%' OR password LIKE '%" & txtcari.Text & "%' OR nama_petugas LIKE '%" & txtcari.Text & "%' OR hak LIKE '%" & txtcari.Text & "%'  "
